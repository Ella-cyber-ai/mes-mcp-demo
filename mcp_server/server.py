...
def get_production(work_date: str = "", line_name: str = "") -> str:
    """생산 실적을 조회합니다. work_date: 날짜(YYYY-MM-DD) 또는 연월(YYYY-MM), line_name: 라인명"""
    conn = get_db()
    cur = conn.cursor()

    work_date = clean(work_date)
    line_name = clean(line_name)

    query = "SELECT * FROM production_result WHERE 1=1"
    params = []

    if work_date:
        if len(work_date) == 7:  # YYYY-MM
            query += " AND work_date LIKE ?"
            params.append(f"{work_date}%")
        else:  # YYYY-MM-DD
            query += " AND work_date = ?"
    ...
    query += " ORDER BY work_date DESC LIMIT 30"
...
def get_defect_analysis(year_month: str = "", line_name: str = "") -> str:
    ...
    query = """
        SELECT line_name, COUNT(*) as defect_count,
               SUM(defect_qty) as total_defect_qty, defect_type
        FROM defect_log
...
@mcp.tool()
def get_equipment_status(work_date: str = "", line_name: str = "") -> str:
    ...
    query = """
        SELECT equipment_name, line_name, operation_time,
               downtime, downtime_reason, work_date
        FROM equipment_status WHERE 1=1
    """
    params = []
...
def get_daily_report(work_date: str = "") -> str:
    ...
    cur.execute("""
        ...
        FROM production_result WHERE work_date = ?
        ORDER BY line_name
    """, (work_date,))
    ...
    cur.execute("""
        ...
        FROM defect_log WHERE work_date = ?
        GROUP BY line_name
    """, (work_date,))
